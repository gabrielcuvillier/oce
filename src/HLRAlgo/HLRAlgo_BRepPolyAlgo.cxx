  ...
  if (nbFaceShell > 0) {
    ...
    for (; exface.More(); exface.Next()) {
      ...
      if (!Tr.IsNull()) {
	    ...
	    cout << " HLRAlgo_BRepPolyAlgo::StoreShell : Face ";
...
//  TMultiply(Nod1NrmX,Nod1NrmY,Nod1NrmZ);
...
#ifdef OCCT_DEBUG
                                                                                                                            if (DoError) {
      cout << "HLRAlgo_BRepPolyAlgo::Normal : AverageNormal error";
      cout << endl;
    }
#endif
  }
  ...
  if (orient)
    UpdateAroundNode(iNode, theNodIndices,
                     TData, PISeg, PINod);
...
InitBiPointsWithConnexity(const Standard_Integer e,
  ...
  if (connex) {
    ...
    if (nbConnex == 1) {
	  ...
	  cout << " HLRAlgo_BRepPolyAlgo::InitBiPointsWithConnexity : ";
	...
	cout << "HLRAlgo_BRepPolyAlgo::InitBiPointsWithConnexity : Edge ";
...
MoveOrInsertPoint(HLRAlgo_ListOfBPoint &List,
  ...
  if (ins3 && mP3P1) {                             // P1 ---> P3
	...
	cout << " HLRAlgo_BRepPolyAlgo::MoveOrInsertPoint : ";
...
#ifdef OCCT_DEBUG
                                                                                                                              if (DoError) {
	cout << "HLRAlgo_BRepPolyAlgo::ChangeNode between " << ip1;
	cout << " and " << ip2 << endl;
      }
#endif
    }
...
#ifdef OCCT_DEBUG
                                                                                                                              if (DoError) {
	cout << "HLRAlgo_BRepPolyAlgo::ChangeNode between " << ip2;
	cout << " and " << ip1 << endl;
      }
#endif
    }
...
#ifdef OCCT_DEBUG
                                                                                                                            if (DoTrace) {
      cout << "HLRAlgo_BRepPolyAlgo::OrientTriangle : OnOutL";
...
#ifdef OCCT_DEBUG
                                                                                                                              if (DoTrace) {
	cout << "HLRAlgo_BRepPolyAlgo::OrientTriangle : Flat";
	cout << " triangle " << iTri << endl;
      }
#endif
      theTriangle.Flags |= HLRAlgo_PolyMask_FMskFlat;
...
#ifdef OCCT_DEBUG
                                                                                                                          if (DoError) {
    cout << "HLRAlgo_BRepPolyAlgo::Triangles : error";
...
//    Standard_Integer i[5];
	...
	cout << "HLRAlgo_BRepPolyAlgo::UpdateEdgesBiPoints : error ";
  ...
  for (f = 1; f <= nbFace; f++) {
    if (!(*pid).IsNull()) {
      ...
      for (i = 1; i <= nbT; i++) {
	    ...
	    cout << "HLRAlgo_BRepPolyAlgo::ReverseBackTriangle :";
	    cout << " face " << f << setw(6);
