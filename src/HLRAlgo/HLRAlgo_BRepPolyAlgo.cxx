...
IMPLEMENT_STANDARD_RTTIEXT(HLRAlgo_BRepPolyAlgo,Standard_Transient)
...
#endif
//=======================================================================
//function : HLRAlgo_BRepPolyAlgo
//purpose  :
//=======================================================================

HLRAlgo_BRepPolyAlgo::HLRAlgo_BRepPolyAlgo () :
...
HLRAlgo_BRepPolyAlgo::HLRAlgo_BRepPolyAlgo (const Handle(HLRAlgo_BRepPolyAlgo)& A)
{
myDebug      = A->Debug();
myAngle      = A->Angle();
...
HLRAlgo_BRepPolyAlgo::HLRAlgo_BRepPolyAlgo (const TopoDS_Shape& S) :
    myDebug     (Standard_False),
    myAngle     (5 * M_PI / 180.),
    myTolSta    (0.1),
    myTolEnd    (0.9),
...
TopoDS_Shape & HLRAlgo_BRepPolyAlgo::Shape (const Standard_Integer I)
{
  Standard_OutOfRange_Raise_if (I == 0 || I > myShapes.Length(),
                                "HLRAlgo_BRepPolyAlgo::Shape : unknown Shape");
  return myShapes(I);
}
...
void HLRAlgo_BRepPolyAlgo::Remove (const Standard_Integer I)
{
  Standard_OutOfRange_Raise_if (I == 0 || I > myShapes.Length(),
                                "HLRAlgo_BRepPolyAlgo::Remove : unknown Shape");
  myShapes.Remove(I);
  myAlgo->Clear();
  myEMap.Clear();
  myFMap.Clear();
...
Standard_Integer HLRAlgo_BRepPolyAlgo::Index (const TopoDS_Shape& S) const
{
...
Handle(HLRAlgo_PolyAlgo) HLRAlgo_BRepPolyAlgo::Algo () const
{
  return myAlgo;
}
...
void HLRAlgo_BRepPolyAlgo::Update ()
{
  myAlgo->Clear();
  myEMap.Clear();
  myFMap.Clear();
...
TopoDS_Shape HLRAlgo_BRepPolyAlgo::MakeShape () const
{
  Standard_Integer n = myShapes.Length();
...
Standard_Integer
HLRAlgo_BRepPolyAlgo::InitShape (const TopoDS_Shape& Shape,
...
void HLRAlgo_BRepPolyAlgo::StoreShell (const TopoDS_Shape& Shape,
  ...
  if (nbFaceShell > 0) {
    ...
    for (; exface.More(); exface.Next()) {
      ...
      if (!Tr.IsNull()) {
	    ...
	    std::cout << " HLRAlgo_BRepPolyAlgo::StoreShell : Face ";
...
Standard_Boolean HLRAlgo_BRepPolyAlgo::
...
#ifdef OCCT_DEBUG
    if (DoError) {
      std::cout << "HLRAlgo_BRepPolyAlgo::Normal : AverageNormal error";
      std::cout << std::endl;
    }
#endif
  }
...
Standard_Boolean
HLRAlgo_BRepPolyAlgo::AverageNormal(const Standard_Integer iNode,
                           ...
                           const Standard_Boolean connex)
{
  ...
  if (connex) {
    ...
    if      (nbConnex == 1) {
	  ...
	  std::cout << " HLRAlgo_BRepPolyAlgo::InitBiPointsWithConnexity : ";
	...
	std::cout << "HLRAlgo_BRepPolyAlgo::InitBiPointsWithConnexity : Edge ";
...
if      (Nod11Indices.Edg1 == e) Nod11RValues.PCu1 = U3;
	...
	std::cout << " HLRAlgo_BRepPolyAlgo::MoveOrInsertPoint : ";
...
void
HLRAlgo_BRepPolyAlgo::InsertOnOutLine (TColStd_Array1OfTransient& PID)
{
  HLRAlgo_Array1OfTData* TData2 = 0;
  HLRAlgo_Array1OfPISeg* PISeg2 = 0;
  HLRAlgo_Array1OfPINod* PINod2 = 0;
...
void
HLRAlgo_BRepPolyAlgo::CheckFrBackTriangles (HLRAlgo_ListOfBPoint& List,
                                        TColStd_Array1OfTransient& PID)
{
...
void HLRAlgo_BRepPolyAlgo::ChangeNode (const Standard_Integer ip1,
                                   const Standard_Integer ip2,
...
#ifdef OCCT_DEBUG
      if (DoError) {
	std::cout << "HLRAlgo_BRepPolyAlgo::ChangeNode between " << ip1;
...
#ifdef OCCT_DEBUG
      if (DoError) {
	std::cout << "HLRAlgo_BRepPolyAlgo::ChangeNode between " << ip2;
...
void
#ifdef OCCT_DEBUG
HLRAlgo_BRepPolyAlgo::OrientTriangle(const Standard_Integer iTri,
#else
HLRAlgo_BRepPolyAlgo::OrientTriangle(const Standard_Integer,
#endif
...
#ifdef OCCT_DEBUG
    if (DoTrace) {
      std::cout << "HLRAlgo_BRepPolyAlgo::OrientTriangle : OnOutL";
...
#ifdef OCCT_DEBUG
      if (DoTrace) {
	std::cout << "HLRAlgo_BRepPolyAlgo::OrientTriangle : Flat";
	std::cout << " triangle " << iTri << std::endl;
      }
#endif
      theTriangle.Flags |=  HLRAlgo_PolyMask_FMskFlat;
...
Standard_Boolean
HLRAlgo_BRepPolyAlgo::Triangles(const Standard_Integer ip1,
                            const Standard_Integer ip2,
                            HLRAlgo_PolyInternalNode::NodeIndices& Nod1Indices,
                            HLRAlgo_Array1OfPISeg*& PISeg,
...
#ifdef OCCT_DEBUG
  if (DoError) {
    std::cout << "HLRAlgo_BRepPolyAlgo::Triangles : error";
...
void
HLRAlgo_BRepPolyAlgo::UpdateOutLines (HLRAlgo_ListOfBPoint& List,
                                  TColStd_Array1OfTransient& PID)
{
  Standard_Integer f;
  Standard_Integer nbFace = myFMap.Extent();
...
//    Standard_Integer i[5];
	...
	std::cout << "HLRAlgo_BRepPolyAlgo::UpdateEdgesBiPoints : error ";
...
void
HLRAlgo_BRepPolyAlgo::UpdatePolyData (TColStd_Array1OfTransient& PD,
                                  TColStd_Array1OfTransient& PID,
                                  const Standard_Boolean closed)
{
  ...
  for (f = 1; f <= nbFace; f++) {
    if (!(*pid).IsNull()) {
      ...
      for (i = 1; i <= nbT; i++) {
	    ...
	    std::cout << "HLRAlgo_BRepPolyAlgo::ReverseBackTriangle :";
...
void
HLRAlgo_BRepPolyAlgo::TMultiply (Standard_Real& X,
                             Standard_Real& Y,
                             Standard_Real& Z,
...
void
HLRAlgo_BRepPolyAlgo::TTMultiply (Standard_Real& X,
                              Standard_Real& Y,
                              Standard_Real& Z,
                              const Standard_Boolean VPO) const
{
...
void
HLRAlgo_BRepPolyAlgo::TIMultiply (Standard_Real& X,
                              Standard_Real& Y,
                              Standard_Real& Z,
                              const Standard_Boolean VPO) const
{
...
HLRAlgo_BiPoint::PointsT& HLRAlgo_BRepPolyAlgo::Hide (
...
HLRAlgo_BiPoint::PointsT& HLRAlgo_BRepPolyAlgo::Show (
    TopoDS_Shape& S,
    Standard_Boolean& reg1,
    Standard_Boolean& regn,
    Standard_Boolean& outl,
...
TopoDS_Shape
HLRAlgo_BRepPolyAlgo::OutLinedShape (const TopoDS_Shape& S) const
{
  TopoDS_Shape Result;
