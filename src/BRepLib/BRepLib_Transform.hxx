...
// NOTE:
// GAB 2019: Copied from BRepBuilderAPI_Transform.hxx, so that some AIS classes does not depends on TKTopAlgo, but rather on TKTopBase

#ifndef _BRepLib_Transform_HeaderFile
#define _BRepLib_Transform_HeaderFile
...
class BRepLib_Transform  : public BRepLib_ModifyShape
{
 public:
  ...
  //! to define the shape to transform.
      Standard_EXPORT BRepLib_Transform(const gp_Trsf& T);
  ...
  //! duplication of <S>.
  Standard_EXPORT BRepLib_Transform(const TopoDS_Shape& S, const gp_Trsf& T, const Standard_Boolean Copy = Standard_False);
...
#endif // _BRepLib_Transform_HeaderFile
